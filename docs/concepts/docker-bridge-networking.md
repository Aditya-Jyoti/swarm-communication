...
```
   ...
   ┌────────────────────────────────────────────────────────────────┐
   │                                                                │
   │   eth0 (192.168.1.50)  ◄── real NIC, the way out               │
   │     ▲                                                          │
   │     │ routing + netfilter (nat POSTROUTING: MASQUERADE)        │
   │     │                                                          │
   │   br-1a2b3c4d5e6f   172.28.0.1/16   ◄── the user-defined bridge│
   │     ├── veth8f2a1c   (peer of node-1's eth0)                   │
   │     ├── vethc41d09   (peer of node-2's eth0)                   │
   │     └── veth77e0ba   (peer of control-center's eth0)           │
   └──────┬──────────────┬─────────────────┬────────────────────────┘
          │              │                 │
   ┌──────┴─────┐ ┌──────┴─────┐  ┌────────┴────────┐
   │  NETNS     │ │  NETNS     │  │  NETNS          │
   │  node-1    │ │  node-2    │  │  control-center │
   │            │ │            │  │                 │
   │ eth0       │ │ eth0       │  │ eth0            │
   │ 172.28.0.2 │ │ 172.28.0.3 │  │ 172.28.0.4      │
   │ lo         │ │ lo         │  │ lo              │
   │            │ │            │  │                 │
   │ default    │ │ default    │  │ default via     │
   │ via .0.1   │ │ via .0.1   │  │ 172.28.0.1      │
   └────────────┘ └────────────┘  └─────────────────┘
```
